...
public class SquareSound extends SoundParam{
	...
	//					double angle = i / (SAMPLE_RATE / hz) * 2.0 * Math.PI;
						double provisionalAngle = ((t * super.getFREQ(keynum) * 2) % 2) * 2 - 1; //矩形波
						double angles = provisionalAngle;
						buf[i] = (byte)(Math.sin(angles) * 127.0 * vol);
					}
	...
	//					double angle = i / (SAMPLE_RATE / hz) * 2.0 * Math.PI;
						double provisionalAngle = ((t * super.getFREQ(keynum) * 2) % 2) * 2 - 1; //矩形波
						double angles = provisionalAngle;
						buf[i] = (byte)(Math.sin(angles) * 127.0 * super.getVol());
					}
