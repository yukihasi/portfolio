...
public class SineSound extends SoundParam{
	...
	private byte[] buf;
	 ...
	 public void createBufs(int msecs, double vol)
				...
				for(int i = 0; i<super.getFREQS().length; i++) {
					...
					for (int j=0; j<bufs[i].length; j++) {
						...
						double provisionalAngle = 2 * Math.PI * super.getFREQ(i) * t;   //正弦波
	 ...
	 public void createBuf(int msecs, double vol,int keynum)
				throws LineUnavailableException {
					buf = new byte[(int)super.getSample_rate() * msecs / 1000];
					for (int i=0; i<buf.length; i++) {
						double t= i / super.getSample_rate();
						double provisionalAngle = 2 * Math.PI * super.getFREQ(keynum) * t;   //正弦波
						double angle = provisionalAngle;
	 ...
	 public void createBuf(int keynum)
				throws LineUnavailableException {
